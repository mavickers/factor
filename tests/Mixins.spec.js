...
describe("Mixins", () => {
    it("should determine basic mixins", () => {
        ...
        expect(Utilities.isInheriting(Model1, Describable)).toBeTrue();
        expect(Utilities.isInheriting(Model1, "Describable")).toBeTrue();
        expect(Utilities.isInheriting(Model2, Describable)).toBeTrue();
        expect(Utilities.isInheriting(Model2, "Describable")).toBeTrue();
        expect(Utilities.isInheriting(Model2, Mappable)).toBeTrue();
        expect(Utilities.isInheriting(Model2, "Mappable")).toBeTrue();

        // Classes() calls Mixins and should add to the
        // target class' inheritance
        expect(Utilities.isInheriting(Model4, Describable)).toBeTrue();
        expect(Utilities.isInheriting(Model4, "Describable")).toBeTrue();

        expect(Utilities.isInheriting(Model1, Configurable)).toBeFalse();
        expect(Utilities.isInheriting(Model1, "Configurable")).toBeFalse();
        expect(Utilities.isInheriting(Model4, Configurable)).toBeFalse();
        expect(Utilities.isInheriting(Model4, "Configurable")).toBeFalse();
    });
    ...
    it("should determine complicated mixins", () => {
        ...
        expect(Utilities.isInheriting(Model1, Class1)).toBeTrue();
        expect(Utilities.isInheriting(Model1, "Class1")).toBeTrue();
        expect(Utilities.isInheriting(Model1, Class2)).toBeTrue();
        expect(Utilities.isInheriting(Model1, "Class2")).toBeTrue();
        expect(Utilities.isInheriting(Model1, Class3)).toBeFalse();
        expect(Utilities.isInheriting(Model1, "Class3")).toBeFalse();

        expect(Utilities.isInheriting(Model2, Class1)).toBeTrue();
        expect(Utilities.isInheriting(Model2, "Class1")).toBeTrue();
        expect(Utilities.isInheriting(Model2, Class2)).toBeFalse();
        expect(Utilities.isInheriting(Model2, "Class2")).toBeFalse();

        expect(Utilities.isInheriting(Model3, Class1)).toBeTrue();
        expect(Utilities.isInheriting(Model3, "Class1")).toBeTrue();
        expect(Utilities.isInheriting(Model3, Class2)).toBeTrue();
        expect(Utilities.isInheriting(Model3, "Class2")).toBeTrue();

        // these next two will be false just because
        expect(Utilities.isInheriting(Model3, Class3)).toBeFalse();
        expect(Utilities.isInheriting(Model3, "Class3")).toBeFalse();

        // the following two are true because Model4 extends
        // Class3 which is decorated with Class2
        expect(Utilities.isInheriting(Model4, Class2)).toBeTrue();
        expect(Utilities.isInheriting(Model4, "Class2")).toBeTrue();
    });
})
